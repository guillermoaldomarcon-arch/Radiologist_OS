...
_NEGATION_PATTERN = re.compile(
    r"\b(sin evidencia de|no se observa|no se identifica|sin)\b",
    re.IGNORECASE,
)
...
def _extract_with_rules(sentence: str) -> Optional[Finding]:
    ...
    # Rule only fires when we have an organ hint AND at least one of
    # (measurement, laterality, explicit negation). Anything weaker
    # is left for the AI fallback rather than guessed here.
    if organ is None:
        return None
    if not (measurement_match or laterality_match or negation_match):
        return None
